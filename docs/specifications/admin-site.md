...
导航图标编辑复用统一图片资源选择器，可以从站点已整理的导航图标目录中选择，也可以上传自定义图标。图标是条目自身属性，不由排序位置推导。管理端对透明图标启用自适应预览背景；该行为仅用于后台可见性辅助，不改变图标文件和公开站页面背景。
...
- 网站属性表格仅紧凑展示当前值，文本/整数/布尔/JSON/图片路径等属性值均通过独立值编辑弹窗维护；
- 网站属性支持 INTEGER，`HOME_CAROUSEL_INTERVAL_SECONDS` 以正整数秒维护；
- 当前阶段无登录/角色/权限实现；
- Browser E2E 覆盖导航收起、图标操作可访问名称/Tooltip、网站属性弹窗值编辑、图片策略、属性元数据分组、整数校验、轮播参数及既有核心路径。