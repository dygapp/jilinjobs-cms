...
上述四项是侧边栏分组标题，不增加额外点击层级。管理端保持独立 `/admin/` SPA 和统一 Application Shell。技术路由 `/pages`、`/page-groups`、`/advertisements` 等可以继续保留；产品界面分别使用“单页 / 单页分组 / 宣传展示”等业务术语。
...
栏目管理本身继续使用整页树形 Table 直接维护栏目树、Alias、排序、状态及新增子栏目等结构信息；当前阶段不额外增加与右侧树表格重复的左侧栏目树。只有当栏目规模或节点属性管理复杂度明显增长时，才评估升级为左树 + 右侧节点详情的 Master–Detail 结构。
...
当前正式基线内置 `MAIN`（主导航）、`HOME_SHORTCUT`（首页快捷入口）、`HOME_QUICK`（首页快速导航）三个导航位置；V8 过渡期引入的 `SERVICE`（服务入口）和 `SITE`（网站导航）仅用于兼容旧模型，现已退出 Authority 并必须从升级数据库和管理端列表中清理。网站导航/友情链接数据由通用列表承担，不得为兼容旧模型继续保留重复导航位置。
...
- 管理端侧边栏按“内容管理 / 内容结构 / 运营展示 / 站点设置”组织八类入口，不增加父级点击层级；
- 产品界面使用“单页管理 / 单页 / 单页分组”，技术层可继续使用 `Page / PageGroup`；
- 单页管理提供“全部单页 / 独立单页 / 单页分组”左侧导航，右侧只展示当前组织上下文中的单页；
- 在具体单页分组上下文新增时默认带入该分组，在独立单页上下文新增时默认不分组；
- 文章管理提供左侧栏目树和“全部文章”，父栏目筛选包含全部后代栏目文章；
- 文章新增/编辑使用层级 TreeSelect 选择所属栏目，并在当前栏目上下文新增时默认带入当前栏目；
- 栏目管理继续使用整页树形 Table，不重复增加左侧栏目树；
- 导航管理按位置 + 树形结构呈现，左侧位置操作体验与列表/展示位一致；
- 正式基线显示 `MAIN`、`HOME_SHORTCUT`、`HOME_QUICK`，不再显示兼容占位 `SERVICE`、`SITE`；
...
- 网站属性可维护定义和值并进行类型校验；
- 当前阶段无登录/角色/权限实现；
- 既有文章、栏目、单页、静态资源管理能力无回归；
- Browser E2E 覆盖主导航分组、文章栏目树父子聚合、TreeSelect、单页分组导航、正式导航位置、导航图标自适应预览、列表图片上传、共享图片复用、宣传展示和网站属性核心路径。
