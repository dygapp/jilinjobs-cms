...
- 通知公告、就业动态、快速导航、招聘公告等多列小板块使用与首页“直播课程”可见标题协调的 Label / Tab 标题：标题文字使用强调色；顶部强调色横条只覆盖 Label / Tab 自身宽度，不铺满整个卡片；Label / Tab 保留左右竖向中性边线，使顶部色条与两侧边线形成完整标签轮廓；
- Label / Tab 之外的 Header 区域继续使用中性分隔线；小板块标题文字不恢复旧的文字下划线强调；顶部左右圆角属于允许的视觉细节，可实现但不作为阻塞验收项；
...
- Main 首页通知公告、就业动态、快速导航、招聘公告等多列小板块采用 Label / Tab 标题：文字为强调色，3px 顶部强调色横条只覆盖标签自身宽度，标签左右保留 1px 中性边线；不得把 3px 强调色扩展为整个卡片通栏顶边，且不恢复标题文字下划线；
