...
- 卡片式 Tab 之外的 Header 区域继续使用 1px 中性底部分隔线；Tab 顶部左右允许使用轻微圆角；小板块标题文字不恢复旧的文字下划线强调；
...
- Main 首页通知公告、就业动态、快速导航、招聘公告等多列小板块采用卡片式 Tab 标题：文字为强调色，3px 顶部强调色横条只覆盖 Tab 自身宽度，Tab 左右保留 1px 中性边线、底边与内容区同为白色，Header 余下部分保留 1px 中性底线；不得把 3px 强调色扩展为整个卡片通栏顶边，且不恢复标题文字下划线；
