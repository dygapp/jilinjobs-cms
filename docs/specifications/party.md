...
本文定义主站“中心党建”特殊栏目/专题页面的 WHAT / WHY。通用 CMS 模型继续以 `docs/requirements/information-publishing.md`、`docs/specifications/cms-core.md` 为准；公开前端共享导航与页脚遵循 ADR-0003 与 `docs/specifications/public-shared-shell.md`。
...
- 四条真实内容线：
  ...
  - 党规党章：`typeCode=dgdz`；
  - 理论学习：`typeCode=llxx`；
- “学习园地”只是“党规党章 + 理论学习”的页面分组，不是第五种内容类型；
- 四个栏目列表使用 `/plist.html?typeCode=...`，且均存在多页历史内容；
...
父栏目用于 CMS 组织和中心党建作用域识别；“学习园地”只属于页面布局。
...
- 列表项继续使用通用标题、图片、URL、打开方式、排序、启停字段；
- 轮播成员属于运营内容，不进入 Flyway；
- 对应本站文章时 URL 指向 `/party/article/{id}`；
...
四条内容线从对应 Column + Article 查询；轮播从 `PARTY_CAROUSEL` 获取，不从全站文章或前端静态数组拼装。
...
Party-owned 视觉仅包括 Banner、内容 Frame、轮播和内容区块模板。
...
Mobile 必须正常响应式，无固定 1200px 横向溢出。

## 7. 数据与历史迁移

Flyway 只负责稳定结构：父栏目、四个子栏目、轮播容器及必要的稳定结构迁移；历史文章、正文图片、附件和轮播成员由 EU-29 独立迁移机制处理。
