...
本 Rule 的目标是减少不必要的人工桥接，不削弱 Human Authority。工具可用不等于自动获得授权；merge、release、deploy、破坏性操作、产品决策、架构决策、权限与安全边界继续由各自 Authority 决定。
...
- 所需事实是否已存在于 Repository、Issue / PR、Actions、日志、文件、当前 Authority 或其他可信 Evidence 中，可以直接恢复；
...
- 高影响、难逆的 business / architecture / integration 决定明确保留给 Human Authority；
...
不得为了减少交互而猜测 Product / Domain intent，也不得用“自动化优先”绕过权限、凭证、安全或 Human Authority。
