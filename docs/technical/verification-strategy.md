...
公开站点与管理端分别形成独立 build artifact 和验证入口；不得用其中一个应用的成功构建替代另一个应用的验证。
...
如果验证 Artifact 与当前更高优先级 Authority / Specification 冲突，应修正拥有过期断言的验证层，不修改产品实现去恢复已被取代的旧行为。Workflow 优先验证构建、运行环境、服务健康、HTTP/API 可达与正式测试套件，不把大量具体产品展示语义重复维护为第二套硬编码契约。
...
如果某个 Artifact 本身构成必要 Verification Evidence，upload step `success` 不能单独证明 Artifact 实体存在；必须重新读取当前 Run 的 Artifact 集合并核对 Artifact 名称、Run 和 Head SHA。
...
1. 确认 Evidence Commit 是当前目标提交祖先，并取得完整 compare diff；
2. 逐项说明差异为什么不会改变该 Claim 所覆盖的行为、环境、数据、资源或人工判断对象；
3. 与该 Claim 相关的 Authority、Requirement、Specification、Architecture、Acceptance、Workflow、Runtime Configuration、Migration、Fixture 和版本化资源没有影响性变化；
...
受影响或无法证明不受影响的 Claim 必须重新取得相应验证或 Review。
...
自动测试创建的数据不得默认进入 Human Review；Reset 后应重新验证测试数据已移除、版本化资源已恢复、人工 Fixture 已准备、服务健康且评审地址可访问。Review Environment 对同一 PR/ref 使用 concurrency / cancellation 避免固定 FRP 域名被旧 Head 占用。

### 5.2 Bind Mount Ownership 与可重复恢复

容器可写 host bind mount 时，显式处理 Runtime UID/GID、host ownership/permissions、清理动作身份和版本化基线重新复制。调用 `rm -rf` 本身不构成 Cleanup Evidence；应重新检查目标路径、预期基线内容和后续 Runtime 结果。
...
人工观察是重要 Evidence，但不会仅因来自 Human Review 就自动成为新 Requirement。明确实现缺陷按当前 Scope 修复并重新验证；Product / Requirement / Architecture 级歧义回到相应 Authority；Runtime Problem 进入系统化诊断。

## 7. Visual Fidelity 验证

对于“现网视觉与布局复刻 + 必要技术适配”的要求，采用：
...
参考证据从当前原网站运行时取得，包括必要截图、DOM/computed style、真实静态资源、宽度/颜色/间距/比例及响应式表现。AI Visual Comparison 在人工评审前消除明显结构、颜色、比例、资源、裁切和响应式问题；最终难以机器阈值表达的视觉判断由 Human Review 承担。

## 8. 外部媒体与二进制输入验证

外部二进制/媒体资源进入版本化基线或 Runtime 前至少遵循：
...
4. 集成 Runtime 启动并从 Fresh Database 执行完整 Flyway chain；
5. 验证 `/`、公开 canonical URL、`/admin/` 和 `/api/**` 路由；
6. Browser E2E 验证管理端核心流程；
7. Browser E2E 验证后台变更到公开站展示的跨边界闭环；
8. 收集当前 Head 测试报告、trace / screenshot 等 Current Evidence；
9. 自动验证后恢复数据库和版本化静态资源基线；
10. 注入明确 Human Admin Review Fixture，并验证自动测试残留已清除；
11. 验证外部 Review URL 的公开站与 `/admin/` 均可访问；
12. 进入 Human Admin Review。

### 9.1 当前 CMS 数据契约专项

Requirement V4.6 引入的图片数据策略与网站属性元数据必须至少形成以下机器证据：
...
最终 Completion Evidence 前，必须对本轮实现差异执行一次 Authority-oriented AI Review，至少检查：
...
Actions 中 queued / pending / in_progress 均为中间状态。当结果仍可通过当前授权路径观察时，应进行有界持续观察，直到成功并取得所需证据、失败后完成诊断/修复/重跑，或出现真实权限/业务/架构/Runtime 阻塞。Dispatch / rerun API 返回成功不等于验证完成。