...
具体 package version、源码文件清单、Gradle task、active migration inventory、测试数量、当前 Workflow run 与 Execution Unit 证据 由 Repository implementation、Work lifecycle 或 GitHub 原生状态持有。

## 归档

`archive/` 保存已完成、被取代或已经被长期 owner 接管的 Technical Plan / convergence record，仅用于 traceability。Archive 默认不参与 Fresh Context。

当一个 Feature Technical Plan 完成后，如果其中仍有跨 Feature 长期 HOW，应折回上述 canonical Technical owner；其余完成态 Planning / READY / EU / exact implementation snapshot 退出根级 当前权威内容。
