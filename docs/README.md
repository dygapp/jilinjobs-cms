...
本文件是 `jilinjobs-cms` 的 Documentation Information Architecture 入口，也是 Consumer 普通运行的 **Local Discovery Entry**。它只提供稳定 Authority / resource locator、按需加载边界与本地失败回退，不替代 `AGENTS.md`、产品 Requirement、Project Roadmap、Work lifecycle、Method / Skill procedure 或 GitHub 当前证据，也不缓存第二份 当前状态 truth。
...
来源可信度不使用脱离语义责任的全局分数。判断一个事实能否作为当前依据时，必须同时看 **来源角色、provenance、适用范围、当前有效性 与 语义所有者**；同一来源在不同 claim 上可以具有不同证明力。
...
| Explicit 人工权威 / decision | 对 Human 明确裁决的当前 bounded decision 具有授权价值 | 只覆盖明确问题与范围；若结论需要跨上下文长期成立，必须 promotion 到对应 规范语义所有者，而不是让聊天或 Issue comment 永久替代它 |
| GitHub-native 当前证据 | 对 Branch、PR、Commit、Actions、Review、Issue timeline 等原生瞬时状态具有当前证据价值 | 不长期拥有 Product / Domain / Architecture 事实；长期语义决定 应进入真实 规范语义所有者 |
...
| Analysis / conversation material | source inventory、comparison、scratchpad、未作明确裁决的会话推理与未 promotion 的分析结论 | 默认非 Authority；若形成 长期事实，必须 promotion 到真实 语义所有者，否则在任务结束后退出 Current runtime |
| Unknown / unresolved | provenance、scope、当前有效性 或 owner 尚不能确定 | 不得被提升为当前事实；若会实质改变产品行为或验收，按当前 Method / 人工权威 处理 |

发生冲突时不按“文件更新日期”或“代码比文档真实”机械裁决。先回到该事实的 语义所有者 与适用范围：Current owner 与 Historical / Legacy 冲突时，Historical 只保留 lineage；ADR 与 current Architecture State 表达不同阶段时，以当前 state owner 解释现状、ADR 保留决策历史；implementation / verification 与 Authority 不一致时，必须先判定哪一侧 stale。多个 Current primary owner 对同一语义给出不兼容结论且无法由责任边界消歧时，失败关闭 并升级到对应 Method / 人工权威。

阶段性 source inventory、Requirement extraction / comparison、冲突矩阵与 review batch 默认保留在当前 controlling Issue / 复核证据 中，不因为“有用”就进入长期 Project Knowledge 或建立永久 source catalog。
...
`docs/work/current/README.md` 只拥有 当前执行生命周期 locator，不拥有 规划候选 排序或完整 Roadmap。GitHub PR / Branch / Actions 对各自原生瞬时状态负责；这不构成“GitHub 永远高于本地文件”的通用规则。
