...
本目录保存 `jilinjobs-cms` 当前采用的一等 Consumer-local Method。这里的 README 只服务 Human View；Agent 的 工作类型 → Method 选择由 `docs/project/project-capability-profile.md` 持有，Method 生命周期语义由各自 canonical Method 文件持有。
...
- `review-feedback-cycle.md`：人工评审 Finding 的批量修复、定向验证与 re-review；
- `method-experiment.md`：Consumer-local 方法 / 工程实践实验与 Evidence promotion lifecycle。

Requirement Baseline Establishment、Architecture Clarification 与 AI Development 是不同 工作类型，通过 Return Contract 自然衔接；当前不保留只负责串联 Requirement 与 Architecture 的 `method:software-project-clarification` super-method。
