...
公开 URL 与具体 HTML Entry 解耦。普通栏目 `/column/{alias}`，站内文章 `/article/{id}`，独立单页 `/page/{alias}`，单页分组成员 `/page/{groupAlias}/{alias}`。

面包屑来自栏目/单页分组/单页等业务关系，不从 URL 或某个导航入口机械推导。

普通栏目列表、文章详情、独立单页和业务指南单页分组继续以现网页面主要版式为复刻基准。
...
1. 管理端侧边栏按“内容管理 / 内容结构 / 运营展示 / 站点设置”组织现有八类 CMS 能力，分组不增加点击层级；
...
5. 现网主导航和二级结构按基线初始化并支持树形管理；
6. 导航位置独立维护，管理列表只展示当前选中位置的导航树；导航位置左侧选择/操作体验与列表、宣传展示位保持一致；
7. `HOME_SHORTCUT` 驱动首页五个首屏快捷入口，并由导航条目自身 `iconPath` 驱动图标；调整排序不得造成图标语义错位；
8. `HOME_QUICK` 驱动业务指南快速入口及其图标，前端不重复硬编码成员列表或按数组下标拼接图标路径；
9. 通用列表不再暴露 `LINK / IMAGE_LINK / TEXT` itemType；标题、副标题、图片、URL 等作为独立数据属性维护，具体展示方式由前台页面设计决定；
10. 首页轮播由 `HOME_CAROUSEL` 通用列表驱动，图片用于轮播展示，URL 可以为空且为空时不产生伪链接；
11. 网站导航/友情链接由 `SITE_LINKS` 通用列表驱动，并允许列表项保存可选 Logo；
12. 首页招聘活动横幅由 `HOME_RECRUITMENT_PROMO` 宣传展示位驱动，同一展示位多条当前有效内容能够按展示顺序轮动；
13. 宣传展示 `NO_LINK` 能在保留 URL 的情况下禁止点击，恢复其他打开方式后继续使用原 URL；有效期能够控制公开可见性且过期记录保留在后台；
14. 管理端用户可见名称使用“宣传展示管理 / 展示位 / 展示内容”，内部兼容性技术标识不要求同步破坏性重命名；
15. 宣传展示、通用列表、导航图标、RESOURCE_PATH 网站属性复用统一图片资源选择/上传能力，并写入约定 `/static/uploads/**` 目录；
16. 现有站点导航图标完成语义整理，业务图标对应关系不再依赖物理序号推导；
17. 网站属性支持通用定义与类型化值校验，不依赖编译期 key Enum；
18. NCSS 首页区域使用固定工程集成，不提供 CMS 管理项；
19. 当前阶段不存在基于用户角色/权限的功能限制实现；
20. 栏目、文章、单页、单页分组、公开 URL 和外链文章规则无回归；
21. 静态资源真实媒体验证、路径安全、关键资源保护和回收恢复无回归，导航图标引用进入可识别关键资源保护；
22. Flyway 干净初始化得到完整 CMS 站点骨架并通过后续 migration 完成列表/导航图标模型升级；
23. Backend、Public Site、Admin Frontend 与 Browser Verification 形成 Current Evidence；
24. Human Review Environment 能对新的 CMS 模型、资源上传体验、内容组织和管理端术语进行重新评审。
