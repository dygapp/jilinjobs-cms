...
已经执行的 Flyway migration 不回改；模型调整通过新的后续 migration 收敛。V11 增加 Column/CmsList 图片数据策略并新增首页轮播间隔网站属性，不修改既有 V8/V9/V10 migration。
...
24. 栏目、文章、单页、单页分组、公开 URL 和外链文章规则无回归；
25. 静态资源真实媒体验证、路径安全、关键资源保护和回收恢复无回归，导航图标引用进入可识别关键资源保护；
26. Flyway 干净初始化得到完整 CMS 站点骨架并通过后续 migration 完成导航图标、通用列表、网站属性和图片策略模型升级；
27. Backend、Public Site、Admin Frontend 与 Browser Verification 形成 Current Evidence；
28. Human Review Environment 能对新的 CMS 模型、资源上传体验、网站属性分组、图片数据契约和管理端术语进行重新评审。
