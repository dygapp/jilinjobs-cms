...
同一任务中由相同或高度重叠 task facts 触发、通常共同发现与消费的一组 policy 应优先聚合。只有拆分能稳定减少无关加载 / 错误激活，或存在不同 technology、artifact、risk、lifecycle、semantic owner 时才独立成 Rule。
...
具体 task signals、matching、locator-only 输出和 fail-closed contract 由 `docs/architecture/rule-discovery.md` 单点拥有。