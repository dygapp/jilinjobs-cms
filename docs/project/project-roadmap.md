...
普通开发优先使用 Consumer-local：`AGENTS.md`、`docs/project/development-method.md`、当前 Requirement / Specification / Technical Plan。当前 Validation Baseline 相对正式 Capability Milestone 只增加 1 个 Stable Maintenance 提交，本 Consumer 已选择性固化 Durable Evidence Promotion 与长生命周期 Review Environment owner / lease / stale-run 规则；没有新增 Method Stage、Engineering Discipline、Technology Profile 或 Task-oriented Skill。`agentic-dev` 自身 Project Roadmap、Foundation / Engineering Discipline Expansion 状态、PR / Issue / Experiment 事实不进入本 Roadmap。
...
| EU-30 轮播图方案与实现收敛 | **当前** | LINK / ARTICLE 列表投放、统一轮播配置与共享生命周期、主题教育历史增量和 Party 第二轮播项 ARTICLE 关系已进入实现/验证收敛；等待最终 Current Evidence 与 Human Review |
...
| 2026-09-04 | Validation Baseline 更新到 `agentic-dev master@394d1c3cde04b35940d5e33b7cbcaaf6557678ce`；相对正式 Capability Milestone 只增加 Stable Maintenance，Consumer-local 选择性固化临时证据晋升为持久输入以及长生命周期 Review Environment owner / lease / stale-run 规则，不继承 `agentic-dev` 自身 Issue / Eval / Roadmap 状态 |
| 2026-09-04 | 固化 EU-29 之后的前端后续路线：先执行 EU-30 轮播图方案与实现收敛，再执行 EU-31 浏览器兼容性与 Runtime Guard 收敛；EU-31 已记录 Public / Admin 双标准、Public IE11 备用路径与独立兼容 Workflow 边界 |
...
- EU-29 接受基线确认四条正式内容线：高层声音 `gcsy`、工作动态 `gzdt`、党规党章 `dgdz`、理论学习 `llxx`；“学习园地”是 PartyHome 入口页对后两者的视觉分组，不是独立 CMS 内容类型；
- EU-30 反向追踪轮播 position 2 后新增候选历史栏目 `party-theme-education / 主题教育`（legacy `zhutijiaoyu / 主题教育2023`），完整历史列表 2 条，当前作为 candidateExtension 等待本 EU Human Review；该栏目属于 Party 内容作用域但不新增 PartyHome 第五个固定内容区；
- 党建正式内容继续复用通用 Column + Article：父栏目 `party` 组织 Party 子栏目；同一栏目可混合 INTERNAL / EXTERNAL_LINK；
...
- EU-29 已将 Human Review 接受的历史迁移数据固化为仓库 `data-migrations/party/v1` canonical dataset；固定 Snapshot Run #7 / Artifact 仅作为 provenance，稳定导入、Canonical Verification 与 Review Environment 不再依赖其可用期；EU-30 在同一持久 Canonical Workspace 中以 acceptedSnapshot + candidateExtension 分层表达新增候选，不重新依赖临时采集 Artifact；
...
- 长生命周期单实例 Review Environment 以真实共享 FRP 域名 / 代理作为冲突域，除并发互斥外还需要可观察 owner / lease / stale-run / release policy；有效 Human Review lease 不因新 Head 出现而机械销毁；
...
EU-29 已关闭；当前先完成两个已确定的横向前端单元，再进入一般“公开站点剩余内容与集成收敛”。详细规划见 `docs/work/frontend-follow-up-execution-units.md`；EU-30 当前执行态以 `docs/work/eu30-carousel-convergence.md` 为准。

```text
EU-30 Carousel Architecture & Behavior Convergence：CURRENT
→ EU-31 Browser Compatibility & Runtime Guard Convergence
```

- **EU-30** 已完成方案讨论并进入实现/验证收敛：通用 List 使用 `LINK / ARTICLE` 表达投放；Main / Party 使用 `CAROUSEL_INTERVAL_SECONDS`、`CAROUSEL_MAX_ITEMS` 与共享行为 composable，视觉继续由各 Site 持有；历史 `主题教育2023` 在新版收敛为 `主题教育`，2 条历史记录作为 candidateExtension，Party 轮播 position 2 改为 ARTICLE + 原轮播 Resource。EU-30 仍需最新 Current Evidence、Review Environment 与 Human Review，未关闭。
...
6. `docs/requirements/information-publishing-eu30-amendment.md`
7. `docs/specifications/public-site.md`
8. `docs/specifications/party.md`
9. `docs/architecture/decisions/ADR-0002-public-site-multi-entry-modular-spa.md`
10. `docs/technical/carousel-list-placement.md`
11. `docs/technical/public-site-frontend.md`
12. `docs/technical/party-frontend.md`
13. `docs/work/eu30-carousel-convergence.md`
14. `docs/work/frontend-follow-up-execution-units.md`
15. `docs/technical/verification-strategy.md`
16. 当前 Branch / PR / CI / Runtime Evidence
