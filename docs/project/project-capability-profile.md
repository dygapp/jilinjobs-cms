...
- **adopt + adapt — Human Review capability**：新增 `architecture:human-review` 与 `skill:human-review`，并把条件性调用触点接入 `method:ai-development`、`method:architecture-clarification` 与 `method:requirement-baseline-establishment`；服务普通 Consumer 软件项目的结构化 Markdown 评审、反馈分类与真实 owner 回写，不复制 upstream Human Guide，也不把人工评审与 independent `review-change` 合并。
...
- **adopt + adapt — executable capability instance closure**：吸收 upstream 对 Tool / compute / external-integration capability 的 executable-instance 义务；通用 lifecycle 落到 `architecture:consumer` 与 `method:consumer-upgrade`，当前 Rule Discovery instance 则由本 Profile 明确 local CLI、exact-SHA cloud transport、result / Evidence recovery 与 fail-closed 路径。upstream executable path 不成为 ordinary-runtime fallback。
