...
如果 Fresh Context 的当前用户目标仅为状态恢复 / 检查 / 总结，完成当前 Repository Authority 与必要 GitHub Current Evidence 的只读核验后即停止；不得仅因发现 non-blocking documentation drift 自动创建新的 Branch / PR / Workflow 生命周期。只读停止边界读取 `../../rules/repository/read-only-state-inspection.md`；高成本 CI / Review Runtime / Historical Migration 的激活边界读取 `../../rules/verification/high-cost-runtime-activation.md`。
