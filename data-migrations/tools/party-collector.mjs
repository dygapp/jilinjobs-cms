...
async function collectResource(rawReference, baseUrl, role) {
  ...
  try {
    const response = await fetchResponse(sourceUrl), bytes = Buffer.from(await response.arrayBuffer()), detected = detectMedia(bytes)
...
async function collectInternal(item) {
  ...
  const resources=[]
  for (const image of rich.find('img[src]').toArray()) { const node=$(image), rawReference=node.attr('src'); if (!rawReference || rawReference.startsWith('data:')) continue; const resource=await collectResource(rawReference,response.url,'BODY_IMAGE'); if (resource) { resources.push(resource); node.attr('src',resource.token) } }
  for (const anchor of rich.find('a[href]').toArray()) { const node=$(anchor), rawReference=node.attr('href'); if (!rawReference) continue; let parsed; try { parsed=new URL(rawReference,response.url) } catch { continue }; if (!attachmentExtensions.has(path.extname(parsed.pathname).slice(1).toLowerCase())) continue; const resource=await collectResource(rawReference,response.url,'ATTACHMENT'); if (resource) { resources.push(resource); node.attr('href',resource.token) } }
...
async function main() {
  ...
  const home=await collectHomeDiscovery(), listItems=[], scopeReport=[]
