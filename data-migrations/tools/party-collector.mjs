...
async function fetchText(url) { const response = await fetchResponse(url); return { url: response.url, contentType: response.headers.get('content-type') || '', text: await response.text() } }
function detectMedia(bytes) {
  const b = Buffer.from(bytes); const ascii = (start, end) => b.subarray(start, end).toString('ascii')
...
async function collectResource(rawReference, baseUrl, role) {
  ...
  try {
    const response = await fetchResponse(sourceUrl); const bytes = Buffer.from(await response.arrayBuffer()); const detected = detectMedia(bytes)
    const declared = (response.headers.get('content-type') || '').split(';')[0].trim() || null; const hash = sha256(bytes)
    ...
    const relativePath = `assets/${role === 'BODY_IMAGE' ? 'body' : 'attachments'}/${hash}.${safeName(extension)}`; const outputPath = path.join(outputRoot, relativePath)
    await mkdir(path.dirname(outputPath), { recursive:true }); await writeFile(outputPath, bytes)
    const record = { role, sourceUrl, originalReference:rawReference, token:`${role === 'BODY_IMAGE' ? 'migration-resource' : 'migration-attachment'}://${hash}`, snapshotPath:relativePath, sha256:hash, contentType:detected.contentType || declared, declaredContentType:declared, sizeBytes:bytes.length }
...
function parsePagination(html, typeCode) {
  const $ = cheerio.load(html); const text = normalizeText($('.pagination-wrap').first().text()); const match = text.match(/共\s*(\d+)\s*页[，,]\s*(\d+)\s*条记录/)
  if (!match) throw new Error(`无法识别 ${typeCode} 分页总量：${text}`); return { pageCount:Number(match[1]), total:Number(match[2]), text }
}
function parseListPage(html, typeCode, pageNo) {
  const $ = cheerio.load(html)
  return $('ul.default-list > li.list-item > a.list-item-a').map((_, element) => {
    const anchor=$(element); const href=anchor.attr('href'); const title=normalizeText(anchor.find('.title').text()); const dateText=normalizeText(anchor.find('.date').text()); if (!href || !title) return null
...
function classifyListItem(item, columnAlias) {
  const url=new URL(item.url); const legacyDetail=url.origin === origin && /\/(pdetail|detail)\.html$/.test(url.pathname); const contentId=legacyDetail?url.searchParams.get('content_id'):null
  ...
  const canonical=url.toString(); return { ...item, columnAlias, articleType:'EXTERNAL_LINK', contentId:null, detailPath:url.pathname, legacyKey:`${item.typeCode}:external:${sha256(canonical)}` }
}
async function collectInternal(item) {
  const response=await fetchText(item.url); const rawName=`${item.typeCode}-${item.contentId}.html`; await writeFile(path.join(rawRoot,'details',rawName),response.text,'utf8'); const $=cheerio.load(response.text)
  const title=normalizeText($('.detail-content-title').first().text()); const tips=$('.detail-content-title-tips > div').map((_,element)=>normalizeText($(element).text())).get(); const sourceText=tips.find(value=>value.startsWith('信息来源：')) || ''; const dateText=tips.find(value=>value.startsWith('发布时间：')) || ''
  const source=normalizeText(sourceText.replace(/^信息来源：/,'')); const detailDateValue=normalizeText(dateText.replace(/^发布时间：/,'')); const detailDate=/^\d{4}-\d{2}-\d{2}$/.test(detailDateValue)?detailDateValue:null; const rich=$('.rich-text-wrap').first()
  ...
  const resources=[]
  for (const image of rich.find('img[src]').toArray()) { const node=$(image); const rawReference=node.attr('src'); if (!rawReference || rawReference.startsWith('data:')) continue; const resource=await collectResource(rawReference,response.url,'BODY_IMAGE'); if (resource) { resources.push(resource); node.attr('src',resource.token) } }
  for (const anchor of rich.find('a[href]').toArray()) { const node=$(anchor); const rawReference=node.attr('href'); if (!rawReference) continue; let parsed; try { parsed=new URL(rawReference,response.url) } catch { continue }; const extension=path.extname(parsed.pathname).slice(1).toLowerCase(); if (!attachmentExtensions.has(extension)) continue; const resource=await collectResource(rawReference,response.url,'ATTACHMENT'); if (resource) { resources.push(resource); node.attr('href',resource.token) } }
  const bodyHtml=rich.html() || ''; const content={ title, source, publishDate:detailDate ?? item.publishDate, bodyHtml, externalUrl:null }
  const fingerprint=sha256(JSON.stringify({ identity:item.legacyKey, target:item.columnAlias, content, resources:resources.map(resource=>[resource.role,resource.sha256]).sort() }))
...
function collectExternal(item) {
  const content={ title:item.title, source:'', publishDate:item.publishDate, bodyHtml:'', externalUrl:item.url }; const fingerprint=sha256(JSON.stringify({ identity:item.legacyKey, target:item.columnAlias, content }))
...
async function collectHomeDiscovery() {
  ...
  const candidates=$('img').map((_,image)=>{ const node=$(image); const parent=node.closest('a[href]'); return { src:node.attr('src')?normalizeUrl(node.attr('src'),url):null, alt:normalizeText(node.attr('alt')) || null, parentHref:parent.attr('href')?normalizeUrl(parent.attr('href'),url):null, className:node.attr('class') || '', parentClassName:parent.attr('class') || '' } }).get()
  await writeFile(path.join(reportRoot,'home-image-candidates.json'),JSON.stringify(candidates,null,2),'utf8'); return { imageCandidates:candidates.length }
}
async function main() {
  ...
  const home=await collectHomeDiscovery(); await writeFile(path.join(outputRoot,'articles.ndjson'),records.map(record=>JSON.stringify(record)).join('\n')+'\n','utf8'); await writeFile(path.join(outputRoot,'resources.ndjson'),[...resourceRecords.values()].map(record=>JSON.stringify(record)).join('\n')+(resourceRecords.size?'\n':''),'utf8')
  const errors=issues.filter(issue=>issue.level==='error'); const reconciliation={generatedAt:new Date().toISOString(),sourceSystem,sourceOrigin:origin,scopes:scopeReport,listedRecords:listItems.length,normalizedRecords:records.length,internal:records.filter(record=>record.target.articleType==='INTERNAL').length,external:records.filter(record=>record.target.articleType==='EXTERNAL_LINK').length,resources:resourceRecords.size,unresolved:errors.length,warnings:issues.length-errors.length,home}
  await writeFile(path.join(reportRoot,'reconciliation.json'),JSON.stringify(reconciliation,null,2),'utf8'); await writeFile(path.join(reportRoot,'issues.json'),JSON.stringify(issues,null,2),'utf8'); console.log(`EU29_COLLECTION_SUMMARY ${JSON.stringify(reconciliation)}`); if (scopeReport.some(scope=>scope.reportedTotal!==scope.uniqueCount)) throw new Error('栏目分页采集数量与原站报告总量不一致')
}
await main()
