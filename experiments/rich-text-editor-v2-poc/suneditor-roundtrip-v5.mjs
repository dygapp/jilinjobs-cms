...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/health') return send(res, 200, 'application/json; charset=utf-8', JSON.stringify({ ok: true, suneditor: version, harness: 5 }))
...
server.listen(port, host, () => {
  console.log(`SunEditor ${version} legacy-image round-trip harness V5: http://${host}:${port}/`)
  console.log('V5 keeps the editable component bridge and rehydrates SunEditor image size state after load.')
})
...
function buildHtml() {
  return `<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SunEditor Legacy Image Round-trip V5</title>
...
<h1>SunEditor ${version} — Legacy Image Round-trip V5</h1>
<div class="muted">V5 修正最小样本图片源，并在 SunEditor 完成 setContents 后重新写入实际 Figure 尺寸状态。</div>
<section class="panel"><b>本轮只看三个条件：</b>① 15×15 / 16×16 是否实际按对应像素显示；② 点击 legacy 图片是否仍弹出图片工具面板；③ 模拟 CMS HTML 保存并重新加载后是否仍保持前两项。</section>
...
const STORAGE_KEY='jilinjobs.suneditor.roundtrip.v5';let editor=null;const original={};
...
function imageStates(raw){const t=document.createElement('template');t.innerHTML=raw;return [...t.content.querySelectorAll('img')].map(img=>({width:dimension(img.style.width)||dimension(img.getAttribute('width')),height:dimension(img.style.height)||dimension(img.getAttribute('height')),float:(img.style.float||img.getAttribute('align')||'').toLowerCase()}))}
function prepareForEditor(raw){if(!q('#bridge').checked)return raw;const t=document.createElement('template');t.innerHTML=raw;t.content.querySelectorAll('img').forEach(img=>{if(img.closest('.se-component'))return;const w=dimension(img.style.width)||dimension(img.getAttribute('width'));const h=dimension(img.style.height)||dimension(img.getAttribute('height'));if(w){img.style.width=w;img.setAttribute('width',attrDimension(w))}if(h){img.style.height=h;img.setAttribute('height',attrDimension(h))}img.setAttribute('data-se-size',(w||'auto')+','+(h||'auto'));const wrapper=document.createElement('span');const float=(img.style.float||img.getAttribute('align')||'').toLowerCase();const align=float==='left'||float==='right'||float==='center'?float:'none';wrapper.className='se-component se-inline-component se-image-container __se__float-'+align;img.replaceWith(wrapper);wrapper.appendChild(img)});return t.innerHTML}
function rehydrateAfterLoad(raw){if(!q('#bridge').checked)return;const states=imageStates(raw);const surface=q('.se-wrapper-wysiwyg[contenteditable="true"]');if(!surface)return;const images=[...surface.querySelectorAll('img')];images.forEach((img,index)=>{const state=states[index];if(!state)return;const w=state.width;const h=state.height;if(w){img.style.width=w;img.setAttribute('width',attrDimension(w))}if(h){img.style.height=h;img.setAttribute('height',attrDimension(h))}img.setAttribute('data-se-size',(w||'auto')+','+(h||'auto'));const component=img.closest('.se-component.se-image-container');if(component){component.classList.remove('__se__float-none','__se__float-left','__se__float-center','__se__float-right');const align=state.float==='left'||state.float==='right'||state.float==='center'?state.float:'none';component.classList.add('__se__float-'+align)}const figure=img.closest('figure');if(figure){if(w)figure.style.width=w;if(h)figure.style.height=h}});setStatus('Legacy image bridge 已 post-load rehydrate '+Math.min(states.length,images.length)+' 张图片')}
...
function setRawHtml(raw){if(!editor)return;const value=prepareForEditor(raw);if(typeof editor.setContents==='function')editor.setContents(value);else editor.$.html.set(value);setTimeout(()=>{rehydrateAfterLoad(raw);diagnostics()},350)}
function destroy(){if(!editor)return;try{editor.destroy()}catch{}editor=null}
function rebuild(){destroy();const old=q('#editor');const fresh=document.createElement('textarea');fresh.id='editor';old.replaceWith(fresh);q('#startup-error').textContent='';const migration=q('#v2-migration').checked;try{editor=SUNEDITOR.create('#editor',{plugins:SUNEDITOR.plugins,lang:window.SUNEDITOR_LANG&&window.SUNEDITOR_LANG.zh_cn,height:'430px',v2Migration:migration,buttonList:[['undo','redo'],['bold','italic','underline','strike'],['list','table'],['link','image']],attributeWhitelist:{table:'align|cellpadding|cellspacing',img:'width|height|align'},tagStyles:{td:'width|height',img:'width|height|float'}});q('#mode').textContent='bridge='+q('#bridge').checked+', v2Migration='+migration;setRawHtml(q('#app-html').value||original.minimal||'');setStatus('SunEditor 初始化成功')}catch(error){q('#startup-error').textContent='SunEditor 初始化失败：'+(error?.message||error);console.error(error)}}
...
function diagnostics(){const surface=q('.se-wrapper-wysiwyg[contenteditable="true"]');if(!surface){q('#diagnostics').textContent='未找到可编辑 surface';return}const rows=[...surface.querySelectorAll('img')].map((img,index)=>{const r=img.getBoundingClientRect();const c=getComputedStyle(img);const component=img.closest('.se-component.se-image-container');return{index:index+1,alt:img.getAttribute('alt'),src:img.getAttribute('src'),width:img.getAttribute('width'),height:img.getAttribute('height'),style:img.getAttribute('style'),dataSeSize:img.getAttribute('data-se-size'),componentTag:component?.tagName||null,componentClass:component?.className||null,naturalWidth:img.naturalWidth,naturalHeight:img.naturalHeight,computedWidth:c.width,computedHeight:c.height,renderedWidth:Math.round(r.width*100)/100,renderedHeight:Math.round(r.height*100)/100}});q('#diagnostics').textContent=rows.length?JSON.stringify(rows,null,2):'当前没有图片'}
function load(name){const raw=q('#source-'+name).value;setRawHtml(raw);setTimeout(()=>{capture();setStatus('已加载 '+name)},500)}
function reset(name){q('#source-'+name).value=original[name];meta(name)}
q('#rebuild').onclick=rebuild;q('#capture').onclick=capture;q('#save').onclick=()=>{const app=capture();localStorage.setItem(STORAGE_KEY,app);q('#snapshot').value=app;setStatus('已保存模拟 CMS HTML 快照')};q('#reload-current').onclick=()=>{const raw=q('#app-html').value;setRawHtml(raw);setTimeout(capture,500)};q('#reload-snapshot').onclick=()=>{const raw=localStorage.getItem(STORAGE_KEY)||'';if(!raw){setStatus('没有浏览器快照');return}q('#app-html').value=raw;setRawHtml(raw);setTimeout(capture,500)};q('#clear').onclick=()=>{localStorage.removeItem(STORAGE_KEY);q('#snapshot').value='';setStatus('已清除快照')};q('#copy').onclick=async()=>{await navigator.clipboard.writeText(q('#app-html').value);setStatus('已复制模拟 CMS HTML')};
...
(async()=>{await initFixtures();q('#snapshot').value=localStorage.getItem(STORAGE_KEY)||'';q('#app-html').value=original.minimal;rebuild();window.__roundtripV5={capture,getHtml,setRawHtml,prepareForEditor,normalizeForApp,rehydrateAfterLoad}})().catch(error=>{q('#startup-error').textContent=String(error?.stack||error);console.error(error)})
</script></body></html>`
}
