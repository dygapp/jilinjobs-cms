...
中心党建正式页面、真实栏目、历史内容迁移与最终 Review 已由 EU-26～EU-29 收敛完成；EU-30 Carousel Architecture & Behavior Convergence、EU-31 Database Migration Baseline Convergence、EU-32～EU-35 Admin Governance / Rich Text、EU-36 Public Frontend Source Isolation，以及 Issue #77 下 EU-37～EU-42 Site Package Boundary 均已完成实现、集成与相应 Current Evidence 收口。**Issue #92 — E1～E3 前置 Repository Authority 与 Migration Architecture 收敛路线是当前 Planning Priority。Phase 0 与 Phase 1 Repository Documentation Authority Convergence 已完成；Phase 2A 已由 EU-46 — Backend Application / Core Boundary Foundation 完成 application / shared-core boundary foundation；Phase 2B 已由 EU-47 — Generic Content Migration Application Foundation 建立 site-neutral Canonical Dataset → CMS Runtime capability并完成 Integration / Post-Integration closure。Phase 2C 已独立完成 dependency closure、Requirement / Specification / Technical Planning、`slice-work` 与 `readiness-check`，形成 **EU-48 — Party Migration De-specialization & Compatibility**；Current Ready Execution Unit = EU-48，Readiness = PASS，Execute = NOT STARTED。EU-48 不继承 EU-47 或更早 Unit 的 Execute Authority；只有本 planning/readiness change 集成到 `main` 后，在新的 Fresh Context重新核验 integrated `main`、Issue #92 / #77、EU-48 Authority、Open PR / Actions 与 base drift，且没有 Authority change / blocker时，才允许建立 EU-48自身 Execute baseline并进入 Execute。Phase 3 与 Issue #60 / E1～E3 继续 downstream / blocked，且不得继承 EU-48 Execute Authority。Issue #77 继续承担 Generic CMS Core / JilinJobs Site Package / Historical Migration / Replaceable Public Renderer 四层架构边界。** 后续工作只允许从本仓库 GitHub Issues #57 / #59 / #60 / #77 / #92 的当前 Planning / Requirement Candidates 或当前 Ready Execution Unit 中选择目标，并按 `docs/project/development-method.md` 完成相应状态链；Planning / Requirement Candidate 只有在上游已经 Ready 时，`slice-work` 才形成 Candidate Execution Unit，并可为其分配稳定 Identifier；只有 `readiness-check` PASS 后才晋升为 Ready Execution Unit并允许进入 Execute。Roadmap 顺序、Issue 编号、模型评审或未来 EU 名称都不能替代该状态链。Documentation Authority Map 统一由 `docs/README.md` 承载；`docs/**/archive/**` 与 `docs/work/archive/**` 默认不参与 Fresh Context Current Authority 恢复。不得从其他项目、其他会话、个人记忆、旧预编号路线、惯例或实现便利性中推导、补充或扩大本项目的产品范围。
