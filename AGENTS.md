...
中心党建正式页面、真实栏目、历史内容迁移与最终 Review 已由 EU-26～EU-29 收敛完成；EU-30 Carousel Architecture & Behavior Convergence、EU-31 Database Migration Baseline Convergence、EU-32～EU-35 Admin Governance / Rich Text、EU-36 Public Frontend Source Isolation，以及 Issue #77 下 EU-37～EU-42 Site Package Boundary 均已完成实现、集成与相应 Current Evidence 收口。**Issue #92 — E1～E3 前置 Repository Authority 与 Migration Architecture 收敛路线的 Phase 0～Phase 3 已完成并关闭；Phase 2A / EU-46、Phase 2B / EU-47、Phase 2C / EU-48 均已完成并终止各自 Execute Authority，Phase 3 Canonical Migration Compatibility & E1～E3 Re-entry Gate 已取得 PASS。Issue #60 / E1～E3 已完成第一轮 dependency closure：E1 Main External-link Ownership & Behavior Boundary 以 Planning / Authority closure 收口且没有形成 implementation Unit；E2 已通过 EU-49 — Page Operational Content Ownership & Migration Foundation 完成并关闭；E3 Main Historical Content Collection & Canonical Migration 已完成 Fresh Context source-evidence recovery、必要 Technical Planning、`slice-work` 与第一段 `readiness-check`。E3 现拆分为 EU-50 — Main Source Discovery & Accepted Snapshot Promotion 与 EU-51 — Main Canonical Import, Reconciliation & Human Review Closure；其中 EU-50 readiness = PASS，Current Ready Execution Unit = EU-50，EU-51 仍依赖 EU-50 accepted snapshot 而保持 Candidate / BLOCKED，不具有 Execute Authority。EU-50 只允许 bounded Legacy Source discovery、completeness/classification 与 repository-owned canonical/evidence promotion，不得执行 Runtime import；EU-51 才负责完全离线的 Generic Content Migration、Runtime reconciliation、Public/Admin/Integrated Browser 与 Human Review。Issue #77 继续承担 Generic CMS Core / JilinJobs Site Package / Historical Migration / Replaceable Public Renderer 四层架构边界。** 下一自然 Gate 是对 integrated Planning Authority 重新建立 EU-50 Fresh-context Execute baseline；不得继承 EU-49、EU-48、Phase 3、E1 或尚未 Ready 的 EU-51 Execute Authority。后续工作只允许从本仓库 GitHub Issues #57 / #59 / #60 / #77 的当前 Planning / Requirement Candidates 或当前 Ready Execution Unit 中选择目标，并按 `docs/project/development-method.md` 完成相应状态链；Planning / Requirement Candidate 只有在上游已经 Ready 时，`slice-work` 才形成 Candidate Execution Unit，并可为其分配稳定 Identifier；只有 `readiness-check` PASS 后才晋升为 Ready Execution Unit并允许进入 Execute。Roadmap 顺序、Issue 编号、模型评审或未来 EU 名称都不能替代该状态链。Documentation Authority Map 统一由 `docs/README.md` 承载；`docs/**/archive/**` 与 `docs/work/archive/**` 默认不参与 Fresh Context Current Authority 恢复。不得从其他项目、其他会话、个人记忆、旧预编号路线、惯例或实现便利性中推导、补充或扩大本项目的产品范围。
...
- Fresh Context 的会话切换提示词只承担 **Locator / Handoff** 职责，不并行维护第二份项目 Authority；凡可从当前 GitHub Repository、Roadmap、Consumer-local Method、Requirement / Specification / Technical Plan、Issue / PR / Actions 或 Runtime Evidence 恢复的事实、规则、状态和执行步骤，不得为了“交接完整”再次复制进提示词；完整规则见 `docs/project/development-method.md` §11.1；
...
- 长生命周期单实例 Review Environment 必须将真实共享资源的排他边界与环境生命周期分开治理：定义 owner、lease 取得 / 续期 / 到期 / 释放条件和 stale 判定；自动 Verification 与 Human Review 可以采用不同租约生命周期；保护有效人工评审租约与让新 Head 获取验证环境属于不同目标，由本仓库 Policy 决定优先级，不机械使用 `latest-head-wins`；释放或接管前核对 owner、lease / activity、环境与授权，接管后重新验证目标 Head 与外部地址归属；
...
- Workflow Artifact、远程 Job 输出、临时快照等默认只承担单次运行的证明、传输、诊断或审查职责；当其中的数据、资源或配置被适当 Human / Product Authority 接受，并成为后续稳定重放、迁移、评审或运行输入时，必须显式 Promotion 到 Consumer 可长期发现和维护的持久来源，只晋升后续真实消费者需要的内容，保留 source Run / Head SHA / Artifact identity / digest / 生成规则等必要 provenance 与 integrity，且 Promotion 影响 Head、输入、Workflow、Importer、Review Environment 或 Evidence Claim 时重新取得最终 Current Evidence；不得让长期消费者把会过期的临时 Artifact 作为唯一输入；
