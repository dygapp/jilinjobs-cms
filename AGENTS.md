...
中心党建正式页面、真实栏目、历史内容迁移与最终 Review 已由 EU-26～EU-29 收敛完成；EU-30 Carousel Architecture & Behavior Convergence、EU-31 Database Migration Baseline Convergence、EU-32～EU-35 Admin Governance / Rich Text、EU-36 Public Frontend Source Isolation，以及 Issue #77 下 EU-37～EU-42 Site Package Boundary 均已完成实现、集成与相应 Current Evidence 收口。**Issue #92 — E1～E3 前置 Repository Authority 与 Migration Architecture 收敛路线是当前 Planning Priority。Phase 0 已完成；Phase 1 Repository Documentation Authority Convergence 的 current audit、Authority Clarification、Slice A / EU-43 与 Slice B / EU-44 均已完成实现、集成与 Post-Integration Evidence 收口。Phase 1B 已完成 dependency closure、`slice-work` 与 `readiness-check`，形成 EU-45 — Documentation Information Architecture & Archive Migration；Readiness = PASS，Execute = NOT STARTED。Current Ready Execution Unit：EU-45。EU-45 Execute Authority 只在本 planning/readiness change 集成到 `main` 后成立，并必须由新的 Fresh Context 先重新核验 base drift、Open PR / Actions 与 Issue #92；其授权只覆盖 `docs/work/eu45-documentation-information-architecture.md` 的 Documentation IA / archive / link-repair 范围。EU-44 Execute Authority 已随完成终止，不授权 Phase 1B、Phase 2 或 Issue #60 / E1～E3。Issue #77 继续承担 Generic CMS Core / JilinJobs Site Package / Historical Migration / Replaceable Public Renderer 四层架构边界；EU-45 完成并取得 Post-Integration Current Evidence 后才可关闭 Phase 1 并转入 Phase 2 Planning。Issue #60 / E1～E3 继续保持 downstream Planning Candidates。** 后续工作只允许从本仓库 GitHub Issues #57 / #59 / #60 / #77 / #92 的当前 Planning / Requirement Candidates 或当前 Ready Execution Unit 中选择目标，并按 `docs/project/development-method.md` 完成相应状态链；Planning / Requirement Candidate 只有在上游已经 Ready 时，`slice-work` 才形成 Candidate Execution Unit，并可为其分配稳定 Identifier；只有 `readiness-check` PASS 后才晋升为 Ready Execution Unit并允许进入 Execute。Roadmap 顺序、Issue 编号、模型评审或未来 EU 名称都不能替代该状态链。根 `README.md` 当前仍包含待 EU-45 Execute 收敛的阶段历史 / Authority 索引；若其后续规划表述与本段、当前 Roadmap、EU-45 work artifact 或 Issue #92 的 Planning Authority 冲突，以本文件的 Governance / 当前 Planning Locator 为准，不得据旧 README 状态跳过当前 Gate。不得从其他项目、其他会话、个人记忆、旧预编号路线、惯例或实现便利性中推导、补充或扩大本项目的产品范围。
