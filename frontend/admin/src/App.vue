    ...
    <aside class="admin-sidebar">
      ...
      <nav class="admin-nav" aria-label="内容管理导航">
        <section v-for="section in sections" :key="section.label" class="admin-nav-section" :data-testid="`admin-nav-section-${section.label}`">
          ...
          <router-link v-for="item in section.items" :key="item.to" :to="item.to" class="admin-nav-item" :data-testid="`admin-nav-${item.to.slice(1)}`">
            <span class="admin-nav-icon">{{ item.icon }}</span>
            <span>{{ item.label }}</span>
          </router-link>
        </section>
      </nav>
      <a class="public-site-link" href="/" target="_blank" rel="noopener">查看公开站 ↗</a>
    </aside>
