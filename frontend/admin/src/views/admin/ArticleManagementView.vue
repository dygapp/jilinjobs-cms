...
const columnSelectTree = computed<ColumnSelectNode[]>(() => {
  const toSelectNodes = (items: ColumnTreeNode[]): ColumnSelectNode[] => items.map(item => ({ value: item.id, label: item.name, children: toSelectNodes(item.children) }))
...
function removeAttachment(id: number) { form.attachmentResourceIds = form.attachmentResourceIds.filter(item => item !== id) }
...
async function hydrateResourceNames(ids: Array<number | null>) {
  ...
  await Promise.all(uniqueIds.map(async id => {
    ...
    try { const resource = await getResource(id); resourceNames[id] = resource.originalFilename }
    catch { resourceNames[id] = `资源 #${id}` }
  }))
}
      ...
      <el-form label-width="95px">
        ...
        <el-form-item v-if="form.articleType === 'INTERNAL' && formCoverPolicy !== 'NONE'" label="封面图片" :required="formCoverPolicy === 'REQUIRED'">
          <div class="resource-row">
            <label class="upload-button"><span>{{ form.coverResourceId == null ? '上传封面' : '更换封面' }}</span><input data-testid="cover-input" type="file" accept="image/*" :disabled="uploading" @change="uploadCover"></label>
        ...
        <el-form-item v-if="form.articleType === 'INTERNAL'" label="附件">
          <div class="attachments">
            <label class="upload-button"><span>上传附件</span><input data-testid="attachment-input" type="file" multiple :disabled="uploading" @change="uploadAttachments"></label>
            <div v-for="id in form.attachmentResourceIds" :key="id" class="attachment-chip" :data-testid="`attachment-${id}`"><span>{{ resourceName(id) }}</span><el-button link type="danger" @click="removeAttachment(id)">移除</el-button></div>
          </div>
        </el-form-item>
