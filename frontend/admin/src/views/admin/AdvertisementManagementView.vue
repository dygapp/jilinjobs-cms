...
<template>
  <main class="admin-shell">
    <header class="page-header"><div><p class="eyebrow">宣传与专题展示</p><h1>宣传展示管理</h1><p class="subtitle">通过展示位维护宣传图片；预置展示位属于稳定页面结构，不可删除，展示内容仍可正常运营维护。</p></div><el-button data-testid="add-ad-slot" type="primary" @click="addSlot">新增展示位</el-button></header>
    ...
    <div class="admin-split-layout" :class="{ 'side-panel-collapsed': sideCollapsed }">
      ...
      <el-card class="admin-main-panel" shadow="never">
        ...
        <el-table v-loading="loading" :data="ads" row-key="id" data-testid="advertisement-table">
          ...
          <el-table-column prop="imagePath" label="图片" min-width="200" show-overflow-tooltip />
    ...
    <el-dialog v-model="slotDialog" :title="editingSlot ? '编辑展示位' : '新增展示位'" width="560px"><el-form label-width="100px"><el-form-item label="Code"><el-input v-model="slotForm.code" :disabled="Boolean(editingSlot)" /><div v-if="editingSlotModel?.preset" data-testid="preset-ad-slot-code-hint" style="color:#909399;font-size:12px">预置展示位的 Code 是稳定站点身份，不允许修改。</div></el-form-item><el-form-item label="名称"><el-input v-model="slotForm.name" /></el-form-item><el-form-item label="说明"><el-input v-model="slotForm.description" type="textarea" /></el-form-item><el-form-item label="展示位排序"><el-input-number v-model="slotForm.sortOrder" /></el-form-item><el-form-item label="启用"><el-switch v-model="slotForm.enabled" /></el-form-item></el-form><template #footer><el-button @click="slotDialog=false">取消</el-button><el-button type="primary" :loading="saving" @click="saveSlot">保存</el-button></template></el-dialog>

    <el-dialog v-model="adDialog" :title="editingAd ? '编辑展示内容' : '新增展示内容'" width="680px"><el-form label-width="110px"><el-form-item label="标题"><el-input v-model="adForm.title" /></el-form-item><el-form-item label="图片"><ImageResourcePicker :model-value="adForm.imagePath" :upload-directory="`uploads/displays/${active?.code || 'GENERAL'}`" @update:model-value="value => adForm.imagePath = value || ''" /></el-form-item><el-form-item label="目标地址"><el-input v-model="adForm.url" placeholder="可选；NO_LINK 模式下保留但不生效" /></el-form-item><el-form-item label="点击行为"><el-select v-model="adForm.openMode" style="width:100%" data-testid="advertisement-open-mode"><el-option label="不跳转，仅展示图片" value="NO_LINK" /><el-option label="按目标默认" value="DEFAULT" /><el-option label="当前窗口" value="SAME_WINDOW" /><el-option label="新窗口" value="NEW_WINDOW" /></el-select><div v-if="adForm.openMode === 'NO_LINK'" style="margin-top:6px;color:#909399;font-size:12px">目标地址会被保留，当前仅禁止点击跳转，后续切换点击行为即可恢复。</div></el-form-item><el-form-item label="开始时间"><el-date-picker v-model="adForm.startAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" clearable /></el-form-item><el-form-item label="结束时间"><el-date-picker v-model="adForm.endAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss" clearable /></el-form-item><el-form-item label="展示顺序"><el-input-number v-model="adForm.sortOrder" /></el-form-item><el-form-item label="启用"><el-switch v-model="adForm.enabled" /></el-form-item></el-form><template #footer><el-button @click="adDialog=false">取消</el-button><el-button type="primary" :loading="saving" @click="saveAd">保存</el-button></template></el-dialog>
  </main>
</template>
