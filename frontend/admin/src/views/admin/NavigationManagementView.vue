...
function target(row:CmsNavigation){if(row.targetType==='HOME')return'网站首页';if(row.targetType==='PLACEHOLDER')return'占位';if(row.targetType==='LINK')return row.targetUrl||'-';if(row.targetType==='PAGE')return pages.value.find(p=>p.id===row.targetPageId)?.name||`页面 #${row.targetPageId}`;return columns.value.find(c=>c.id===row.targetColumnId)?.name||`栏目 #${row.targetColumnId}`}
  ...
  <main class="admin-shell">
    ...
    <div style="display:grid;grid-template-columns:230px minmax(0,1fr);gap:16px;align-items:start">
      ...
      <el-card shadow="never">
        ...
        <el-table v-loading="loading" :data="treeData" row-key="id" default-expand-all :tree-props="{children:'children'}" data-testid="navigation-tree-table">
          ...
          <el-table-column label="操作" width="150"><template #default="s"><el-button link type="primary" @click="openEdit(asNavigation(s.row))">编辑</el-button><el-button link type="danger" @click="remove(asNavigation(s.row))">删除</el-button></template></el-table-column>
        </el-table>
      </el-card>
    </div>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId==null?'新增导航':'编辑导航'" width="680px">
      <el-form label-width="100px">
        ...
        <el-form-item v-if="form.targetType==='PAGE'" label="目标页面"><el-select v-model="form.targetPageId" filterable style="width:100%"><el-option v-for="p in pages" :key="p.id" :label="p.name" :value="p.id"/></el-select></el-form-item>
    ...
    <el-dialog v-model="locationDialogVisible" :title="editingLocationCode?'编辑导航位置':'新增导航位置'" width="560px">
      <el-form label-width="100px"><el-form-item label="Code"><el-input v-model="locationForm.code" :disabled="Boolean(editingLocationCode)" placeholder="例如 FOOTER"/></el-form-item><el-form-item label="名称"><el-input v-model="locationForm.name"/></el-form-item><el-form-item label="说明"><el-input v-model="locationForm.description" type="textarea"/></el-form-item><el-form-item label="排序"><el-input-number v-model="locationForm.sortOrder"/></el-form-item><el-form-item label="启用"><el-switch v-model="locationForm.enabled"/></el-form-item></el-form>
