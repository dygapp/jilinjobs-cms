  ...
  <main class="admin-shell">
    <header class="page-header">
      <div>
        ...
        <h1>单页管理</h1>
        <p class="subtitle">维护独立单页、单页分组和呈现模式；公开 URL 由稳定 Alias 生成。</p>
      </div>
    ...
    <div class="page-management-layout">
      <el-card class="page-group-panel" shadow="never">
        ...
        <button
          class="page-group-item"
          :class="{ active: selectedGroup === 'all' }"
          data-testid="page-group-all"
          type="button"
        ...
        <button
          ...
          data-testid="page-group-ungrouped"
          type="button"
          @click="selectGroup('ungrouped')"
        >
          <span>独立单页</span>
          <small>{{ groupCount(null) }}</small>
        </button>
        ...
        >
          <span class="page-group-name">{{ group.name }}</span>
      ...
      <el-card class="page-list-panel" shadow="never">
        <div class="page-list-context" data-testid="page-group-context">
          <div>
            ...
            <span>{{ selectedGroup === 'all' ? '查看全部单页' : selectedGroup === 'ungrouped' ? '不属于任何分组的独立单页' : '当前分组成员' }}</span>
          </div>
          <small>共 {{ filteredPages.length }} 项</small>
        </div>
        <el-table v-loading="loading" :data="filteredPages" data-testid="page-table">
          <el-table-column prop="name" label="单页名称" min-width="170" />
