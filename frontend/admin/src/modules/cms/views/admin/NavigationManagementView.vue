...
type NavigationForm = Omit<NavigationDraft, 'openMode'> & { openMode: '' | '_self' | '_blank' }
const form = reactive<NavigationForm>({ name: '', position: 'MAIN', category: null, targetType: 'COLUMN', targetColumnId: null, targetPageId: null, targetUrl: null, parentId: null, openMode: '', iconPath: null, sortOrder: 0, enabled: true })
...
const editingLocation = computed(() => editingLocationCode.value == null ? null : locations.value.find(i => i.code === editingLocationCode.value) || null)
...
const treeData = computed(() => {
  ...
  map.forEach(item => { if (item.parentId && map.has(item.parentId)) map.get(item.parentId)!.children!.push(item); else roots.push(item) })
  return roots
})
...
function openCreate() { editingId.value = null; Object.assign(form, { name: '', position: activePosition.value, category: null, targetType: 'COLUMN', targetColumnId: null, targetPageId: null, targetUrl: null, parentId: null, openMode: '', iconPath: null, sortOrder: 0, enabled: true }); dialogVisible.value = true }
...
async function save() { saving.value = true; try { form.position = activePosition.value; const draft: NavigationDraft = { ...form, openMode: form.openMode || null }; editingId.value == null ? await createNavigation(draft) : await updateNavigation(editingId.value, draft); dialogVisible.value = false; await refresh() } catch (e) { ElMessage.error(msg(e)) } finally { saving.value = false } }
...
<template>
  <main class="admin-shell">
    <header class="page-header"><div><p class="eyebrow">内容结构</p><h1>导航管理</h1><p class="subtitle">按导航位置维护多级菜单、跳转目标、排序和启停状态。</p></div><el-button data-testid="add-navigation" type="primary" :disabled="!currentLocation" @click="openCreate">新增导航</el-button></header>

    <div class="admin-split-layout" :class="{ 'side-panel-collapsed': sideCollapsed }">
      <el-card class="admin-side-panel" shadow="never">
        <template #header><div class="admin-card-header"><strong>导航位置</strong><el-button link type="primary" data-testid="add-navigation-location" @click="openCreateLocation">新增</el-button></div></template>
        <div class="admin-side-list">
          <div v-for="location in locations" :key="location.code" class="admin-side-row" :class="{ active: activePosition === location.code }" :data-testid="`navigation-location-${location.code}`" @click="selectLocation(location.code)">
            <span class="admin-side-row-main">{{location.name}}<el-tag v-if="location.preset" :data-testid="`preset-navigation-location-${location.code}`" size="small" type="info" style="margin-left:6px">预置</el-tag></span>
            <span @click.stop><el-dropdown trigger="click" @command="command => command === 'edit' ? openEditLocation(location) : removeLocation(location)"><el-button text circle aria-label="导航位置操作"><el-icon><MoreFilled /></el-icon></el-button><template #dropdown><el-dropdown-menu><el-dropdown-item command="edit">编辑</el-dropdown-item><el-dropdown-item command="delete" divided :disabled="location.preset">删除</el-dropdown-item></el-dropdown-menu></template></el-dropdown></span>
          </div>
        </div>
      </el-card>

      <el-card class="admin-main-panel" shadow="never">
        <template #header><div class="admin-card-header"><div class="admin-card-header-title"><AdminPanelToggle :collapsed="sideCollapsed" label="导航位置" @toggle="sideCollapsed = !sideCollapsed" /><div><strong>{{currentLocation?.name || '请选择导航位置'}}</strong><span v-if="currentLocation" style="margin-left:8px;color:#909399">{{currentLocation.code}}</span><el-tag v-if="currentLocation?.preset" size="small" type="info" style="margin-left:8px">预置</el-tag></div></div></div></template>
        <el-table v-loading="loading" :data="treeData" row-key="id" default-expand-all :tree-props="{children:'children'}" data-testid="navigation-tree-table">
          ...
          <el-table-column label="目标" min-width="220"><template #default="s">{{target(asNavigation(s.row))}}</template></el-table-column><el-table-column prop="sortOrder" label="排序" width="80" />
          ...
          <el-table-column label="操作" width="92" fixed="right"><template #default="s"><div class="admin-table-actions"><AdminIconAction label="编辑" :icon="Edit" @click="openEdit(asNavigation(s.row))" /><AdminIconAction v-if="!asNavigation(s.row).preset" label="删除" :icon="Delete" type="danger" @click="remove(asNavigation(s.row))" /></div></template></el-table-column>
        </el-table>
      </el-card>
    </div>

    <el-dialog v-model="dialogVisible" :title="editingId == null ? '新增导航' : '编辑导航'" width="680px">
      <el-form label-width="100px"><el-form-item label="导航位置"><span data-testid="navigation-location-context">{{ currentLocation?.name || activePosition }}</span></el-form-item><el-form-item label="导航名称"><el-input v-model="form.name" /></el-form-item><el-form-item label="图标"><ImageResourcePicker v-model="form.iconPath" upload-directory="uploads/navigation-icons" :preset-options="navigationIconCatalog" adaptive-preview /></el-form-item><el-form-item label="上级菜单"><el-select v-model="form.parentId" clearable style="width:100%"><el-option v-for="i in parentOptions" :key="i.id" :label="i.name" :value="i.id" /></el-select></el-form-item><el-form-item label="目标类型"><el-select v-model="form.targetType" style="width:100%"><el-option v-for="o in targetOptions" :key="o.value" :label="o.label" :value="o.value" /></el-select></el-form-item><el-form-item v-if="form.targetType === 'COLUMN'" label="目标栏目"><el-select data-testid="navigation-column-select" v-model="form.targetColumnId" filterable style="width:100%"><el-option v-for="c in columns" :key="c.id" :label="c.name" :value="c.id" /></el-select></el-form-item><el-form-item v-if="form.targetType === 'PAGE'" label="目标单页"><el-select v-model="form.targetPageId" filterable style="width:100%"><el-option v-for="p in pages" :key="p.id" :label="p.name" :value="p.id" /></el-select></el-form-item><el-form-item v-if="form.targetType === 'LINK'" label="目标地址"><el-input v-model="form.targetUrl" placeholder="站内路径或 HTTP(S) 地址" /></el-form-item><el-form-item label="打开方式"><el-select v-model="form.openMode" style="width:100%"><el-option label="默认" value="" /><el-option label="当前窗口" value="_self" /><el-option label="新窗口" value="_blank" /></el-select></el-form-item><el-form-item label="排序"><el-input-number v-model="form.sortOrder" /></el-form-item><el-form-item label="状态"><el-switch v-model="form.enabled" /></el-form-item></el-form>
    ...
    <el-dialog v-model="locationDialogVisible" :title="editingLocationCode ? '编辑导航位置' : '新增导航位置'" width="560px">
      <el-form label-width="100px"><el-form-item label="位置标识"><el-input v-model="locationForm.code" :disabled="Boolean(editingLocationCode)" placeholder="例如 FOOTER" /><div v-if="editingLocation?.preset" data-testid="preset-navigation-location-code-hint" style="color:#909399;font-size:12px">预置导航位置的标识不可修改。</div></el-form-item><el-form-item label="名称"><el-input v-model="locationForm.name" /></el-form-item><el-form-item label="说明"><el-input v-model="locationForm.description" type="textarea" /></el-form-item><el-form-item label="排序"><el-input-number v-model="locationForm.sortOrder" /></el-form-item><el-form-item label="启用"><el-switch v-model="locationForm.enabled" /></el-form-item></el-form>
