...
import { contentImagePolicyLabel, contentImagePolicyOptions } from '../../cmsEnums'
import {
  ...
  <main class="admin-shell">
    ...
    <div class="admin-split-layout" :class="{ 'side-panel-collapsed': sideCollapsed }">
      ...
      <el-card class="admin-main-panel" shadow="never">
        <template #header><div class="admin-card-header"><div class="admin-card-header-title"><AdminPanelToggle :collapsed="sideCollapsed" label="列表导航" @toggle="sideCollapsed = !sideCollapsed" /><div><strong>{{active?.name || '请选择列表'}}</strong><span v-if="active" style="margin-left:8px;color:#909399">{{active.code}}</span><el-tag v-if="active?.preset" data-testid="active-list-preset" size="small" type="info" style="margin-left:8px">预置</el-tag><el-tag v-if="active" data-testid="active-list-image-policy" size="small" style="margin-left:8px">图片：{{contentImagePolicyLabel(active.imagePolicy)}}</el-tag></div></div><el-button data-testid="add-cms-list-item" type="primary" :disabled="!active" @click="addItem">新增列表项</el-button></div></template>
