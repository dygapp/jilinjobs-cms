...
type CmsListItemForm = Omit<CmsListItemDraft, 'openMode'> & { openMode: '' | '_self' | '_blank' }
const itemForm = reactive<CmsListItemForm>({ sourceType: 'LINK', articleId: null, title: '', subtitle: null, url: null, imagePath: null, imageResourceId: null, openMode: '', sortOrder: 0, enabled: true, extraJson: null })
...
async function addItem() {
  ...
  Object.assign(itemForm, { sourceType: 'LINK', articleId: null, title: '', subtitle: null, url: null, imagePath: null, imageResourceId: null, openMode: '', sortOrder: 0, enabled: true, extraJson: null })
...
async function saveItem() {
  ...
  saving.value = true
  try {
    const draft: CmsListItemDraft = { ...itemForm, openMode: itemForm.openMode || null }
    editingItem.value ? await updateCmsListItem(activeId.value, editingItem.value, draft) : await createCmsListItem(activeId.value, draft)
...
<template>
  <main class="admin-shell">
    <header class="page-header"><div><p class="eyebrow">可复用展示数据</p><h1>列表管理</h1><p class="subtitle">维护可复用的链接和文章投放列表。</p></div><el-button data-testid="add-cms-list" type="primary" @click="addList">新增列表</el-button></header>
    ...
    <el-dialog v-model="itemDialog" :title="editingItem ? '编辑列表项' : '新增列表项'" width="760px">
      <el-form label-width="110px">
        ...
        <template v-if="itemForm.sourceType === 'LINK'">
          ...
          <el-form-item label="目标地址"><el-input v-model="itemForm.url" placeholder="可选" /></el-form-item>
        </template>
        ...
        <template v-else>
          <el-form-item label="关联文章" required>
            ...
            <div v-if="editingItem != null" data-testid="list-item-article-immutable-hint" style="color:#909399;font-size:12px">关联文章创建后不可更改；如需替换，请新建列表项。</div>
          </el-form-item>
          ...
          <el-form-item v-if="active?.imagePolicy !== 'NONE'" label="展示图片" :required="active?.imagePolicy === 'REQUIRED'">
            <div v-if="selectedArticle" style="display:grid;gap:10px;width:100%">
              ...
              <div v-if="effectiveArticleImageId" style="display:flex;align-items:center;gap:10px"><span>当前有效图片：</span><AdaptiveImagePreview :src="resourceContentUrl(effectiveArticleImageId)" :alt="selectedArticle.title" adaptive style="width:120px;height:74px" /><small>{{itemForm.imageResourceId ? '列表专用图片' : '文章主题图片'}}</small></div>
        ...
        <el-form-item label="打开方式"><el-select v-model="itemForm.openMode" style="width:100%"><el-option label="默认" value="" /><el-option label="当前窗口" value="_self" /><el-option label="新窗口" value="_blank" /></el-select></el-form-item>
