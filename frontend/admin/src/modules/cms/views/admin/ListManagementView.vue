...
<template>
  <main class="admin-shell">
    ...
    <div class="admin-split-layout" :class="{ 'side-panel-collapsed': sideCollapsed }">
      ...
      <el-card class="admin-main-panel" shadow="never">
        ...
        <el-table v-loading="loading" :data="items" row-key="id" data-testid="cms-list-item-table">
          ...
          <el-table-column v-if="active?.imagePolicy !== 'NONE'" label="图片" min-width="220"><template #default="scope"><div v-if="asItem(scope.row).imagePath" style="display:flex;align-items:center;gap:10px"><AdaptiveImagePreview :src="asItem(scope.row).imagePath || ''" :alt="asItem(scope.row).title" adaptive style="width:68px;height:42px;flex:none" /><code style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap" :title="asItem(scope.row).imagePath || ''">{{asItem(scope.row).imagePath}}</code></div><span v-else>—</span></template></el-table-column>
    ...
    <el-dialog v-model="dialog" :title="editingList ? '编辑列表' : '新增列表'" width="580px"><el-form label-width="110px"><el-form-item label="Code"><el-input v-model="listForm.code" :disabled="Boolean(editingList)" /><div v-if="editingListModel?.preset" data-testid="preset-list-code-hint" style="color:#909399;font-size:12px">预置列表的 Code 是稳定站点身份，不允许修改。</div></el-form-item><el-form-item label="名称"><el-input v-model="listForm.name" /></el-form-item><el-form-item label="分组"><el-input v-model="listForm.groupCode" /></el-form-item><el-form-item label="图片要求"><el-select v-model="listForm.imagePolicy" data-testid="list-image-policy" style="width:100%"><el-option v-for="option in contentImagePolicyOptions" :key="option.value" :label="option.label" :value="option.value" /></el-select><div style="color:#909399;font-size:12px">仅约束列表项是否保存图片数据，不决定公开页面如何展示。</div></el-form-item><el-form-item label="说明"><el-input v-model="listForm.description" type="textarea" /></el-form-item><el-form-item label="排序"><el-input-number v-model="listForm.sortOrder" /></el-form-item><el-form-item label="启用"><el-switch v-model="listForm.enabled" /></el-form-item></el-form><template #footer><el-button @click="dialog=false">取消</el-button><el-button type="primary" :loading="saving" @click="saveList">保存</el-button></template></el-dialog>

    <el-dialog v-model="itemDialog" :title="editingItem ? '编辑列表项' : '新增列表项'" width="680px"><el-form label-width="100px" :data-testid="active?.imagePolicy === 'NONE' ? 'list-image-disabled' : undefined"><el-form-item label="标题"><el-input v-model="itemForm.title" /><div style="color:#909399;font-size:12px">标题作为后台识别名称保留；前台是否显示由具体页面设计决定。</div></el-form-item><el-form-item label="副标题"><el-input v-model="itemForm.subtitle" /></el-form-item><el-form-item v-if="active?.imagePolicy !== 'NONE'" label="图片" :required="active?.imagePolicy === 'REQUIRED'"><ImageResourcePicker v-model="itemForm.imagePath" :upload-directory="`uploads/lists/${active?.code || 'GENERAL'}`" adaptive-preview /></el-form-item><el-form-item label="目标地址"><el-input v-model="itemForm.url" placeholder="可选；是否需要链接由具体页面使用场景决定" /></el-form-item><el-form-item label="打开方式"><el-select v-model="itemForm.openMode" style="width:100%"><el-option label="按目标默认" value="DEFAULT" /><el-option label="当前窗口" value="SAME_WINDOW" /><el-option label="新窗口" value="NEW_WINDOW" /></el-select></el-form-item><el-form-item label="排序"><el-input-number v-model="itemForm.sortOrder" /></el-form-item><el-form-item label="启用"><el-switch v-model="itemForm.enabled" /></el-form-item></el-form><template #footer><el-button @click="dialog=false">取消</el-button><el-button type="primary" :loading="saving" @click="saveItem">保存</el-button></template></el-dialog>
  </main>
</template>
