...
type AdvertisementForm = Omit<AdvertisementDraft, 'openMode'> & { openMode: '' | '_self' | '_blank' | 'NO_LINK' }
const adForm = reactive<AdvertisementForm>({ title: '', imagePath: '', url: null, openMode: '', startAt: null, endAt: null, sortOrder: 0, enabled: true })
...
function addAd() { editingAd.value = null; Object.assign(adForm, { title: '', imagePath: '', url: null, openMode: '', startAt: null, endAt: null, sortOrder: 0, enabled: true }); adDialog.value = true }
...
async function saveAd() { if (!activeId.value) return; saving.value = true; try { const draft: AdvertisementDraft = { ...adForm, openMode: adForm.openMode || null }; editingAd.value ? await updateAdvertisement(activeId.value, editingAd.value, draft) : await createAdvertisement(activeId.value, draft); adDialog.value = false; await refreshAds() } catch (e) { ElMessage.error(msg(e)) } finally { saving.value = false } }
...
function openModeLabel(mode: string | null) { return mode === 'NO_LINK' ? '不跳转' : mode === '_self' ? '当前窗口' : mode === '_blank' ? '新窗口' : '默认' }
