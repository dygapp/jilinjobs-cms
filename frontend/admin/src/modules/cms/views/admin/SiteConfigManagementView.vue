  ...
  <main class="admin-shell">
    ...
    <el-dialog v-model="valueDialog" :title="`编辑属性值${valueEditingItem ? `：${valueEditingItem.name}` : ''}`" width="620px" data-testid="site-config-value-dialog">
      <el-form v-if="valueEditingItem" label-width="100px">
        ...
        <el-form-item label="属性值"><el-switch v-if="valueEditingItem.valueType === 'BOOLEAN'" :model-value="valueDraft.toLowerCase() === 'true'" @change="value => valueDraft = value === true ? 'true' : 'false'" /><ImageResourcePicker v-else-if="valueEditingItem.valueType === 'RESOURCE_PATH'" :model-value="valueDraft" :upload-directory="`uploads/site-properties/${valueEditingItem.key}`" adaptive-preview @update:model-value="value => valueDraft = value || ''" /><el-input v-else v-model="valueDraft" :data-testid="`site-config-${valueEditingItem.key}`" :type="valueEditingItem.valueType === 'JSON' ? 'textarea' : valueEditingItem.valueType === 'INTEGER' ? 'number' : 'text'" :rows="valueEditingItem.valueType === 'JSON' ? 6 : undefined" /></el-form-item>
