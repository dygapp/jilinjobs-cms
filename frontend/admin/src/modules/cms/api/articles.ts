...
export function listArticles(query:AdminArticleQuery={}){const p=new URLSearchParams({page:String(query.page??0),size:String(query.size??10)});const keyword=query.keyword?.trim();if(keyword)p.set('keyword',keyword);if(query.columnId!=null)p.set('columnId',String(query.columnId));if(query.status)p.set('status',query.status);if(query.articleType)p.set('articleType',query.articleType);return jsonRequest<AdminArticlePage>(`/api/admin/articles?${p}`)}
