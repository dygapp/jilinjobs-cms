...
export async function uploadStaticResource(path:string,file:File,replace=false){const form=new FormData();form.append('file',file);return checked<StaticEntry>(fetch(`/api/admin/static-resources?path=${encodeURIComponent(path)}&replace=${replace}`,{method:'POST',body:form}))}
