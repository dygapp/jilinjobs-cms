...
test('站点基线：初始化静态资源包可见，上传替换删除恢复形成闭环', async ({ page, request }, testInfo) => {
  ...
  await expect(page.getByTestId('static-resource-table')).toContainText('e2e')
  await page.getByRole('button', { name: '进入' }).filter({ visible: true }).first().click()
  await expect(page.getByTestId('static-current-path')).toHaveText('e2e')
  await expect(page.getByTestId('static-resource-table')).toContainText(`${suffix}.png`)
  await expect(page.getByRole('link', { name: '查看/下载' }).first()).toHaveAttribute('href', `/static/${path}`)
