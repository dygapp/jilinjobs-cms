...
test('EU-28：中心党建 Banner 使用本地原站基线资源且不可点击，公共导航和页脚仅覆盖红色主题', async ({ page, request }) => {
  ...
  await expect(page.locator('.party-block-title h2').first()).toHaveCSS('font-size', '24px')
  await expect(page.locator('.party-block-title h2').first()).toHaveCSS('line-height', '32px')
  const partyMore = page.locator('.party-red-tab a').first()
  await expect(partyMore).toHaveCSS('color', 'rgb(132, 145, 161)')
  await expect(partyMore).toHaveCSS('font-size', '13px')
  await expect(partyMore).toHaveCSS('line-height', '18.2px')
  const partyDate = page.locator('.party-article-list time').first()
  await expect(partyDate).toHaveCSS('color', 'rgb(132, 145, 161)')
  await expect(partyDate).toHaveCSS('font-size', '13px')
  await expect(partyDate).toHaveCSS('line-height', '18.2px')
...
test('EU-28：中心党建 Desktop / Mobile 当前视觉截图形成 Current Evidence 且轮播进入对应新闻详情', async ({ page, request }, testInfo) => {
  ...
  try {
    ...
    await expect(page.locator('.party-block-title h2').first()).toHaveCSS('font-size', '24px')
    const mobileCaption = page.locator('.party-carousel-item.active strong')
    await expect(mobileCaption).toHaveCSS('font-size', '14px')
    await expect(mobileCaption).toHaveCSS('line-height', '20px')
    await expect(mobileCaption).toHaveCSS('min-height', '40px')
    await expect(mobileCaption).toHaveCSS('background-color', 'rgba(0, 18, 41, 0.44)')
