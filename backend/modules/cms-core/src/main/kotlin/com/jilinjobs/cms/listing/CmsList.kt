...
class CmsListService(
    ...
    private fun publicList(record: CmsListRecord): PublicCmsList {
        ...
        return PublicCmsList(
            requireNotNull(record.id), record.code, record.name, record.groupCode, policy,
            mapper.findEnabledItems(requireNotNull(record.id)).mapNotNull { row ->
                val item = resolveItem(row, policy)
                if (item.sourceType == CmsListItemSourceType.ARTICLE && item.articleStatus != ArticleStatus.PUBLISHED) return@mapNotNull null
    ...
    private fun normalizeItem(draft: CmsListItemDraft, policy: ContentImagePolicy): CmsListItemDraft {
        ...
        return when (draft.sourceType) {
            CmsListItemSourceType.LINK -> {
                ...
                draft.copy(articleId = null, title = title, subtitle = draft.subtitle?.trim()?.takeIf { it.isNotBlank() }, url = url, imagePath = image, imageResourceId = null, openMode = mode, extraJson = extra)
            }
            CmsListItemSourceType.ARTICLE -> {
                ...
                if (overrideResourceId != null) {
                    val resource = runCatching { resourceService.get(overrideResourceId) }.getOrElse { throw CmsListValidationException("列表项图片资源不存在：$overrideResourceId") }
                ...
                when (policy) {
                    ...
                    ContentImagePolicy.REQUIRED -> if (overrideResourceId == null && coverResourceId == null) throw CmsListValidationException("当前列表要求图片；关联文章没有主题图片，请从正文图片中选择或上传轮播图片")
                    ContentImagePolicy.OPTIONAL -> Unit
                }
                draft.copy(articleId = article.id, title = article.title, subtitle = draft.subtitle?.trim()?.takeIf { it.isNotBlank() }, url = null, imagePath = null, imageResourceId = overrideResourceId, openMode = mode, extraJson = extra)
            }
        }
    }
    ...
    private fun validateExistingItemsForPolicy(listId: Long, policy: ContentImagePolicy) {
        ...
        when (policy) {
            ContentImagePolicy.NONE -> if (items.any { row ->
                row.sourceType == CmsListItemSourceType.LINK.name && !row.imagePath.isNullOrBlank() || row.sourceType == CmsListItemSourceType.ARTICLE.name && row.imageResourceId != null
    ...
    private fun resolveItem(record: CmsListItemRecord, policy: ContentImagePolicy): CmsListItem {
        ...
        if (sourceType == CmsListItemSourceType.LINK) {
            return CmsListItem(requireNotNull(record.id), record.listId, sourceType, null, null, null, record.title, record.subtitle, record.url, record.imagePath, null, null, record.openMode, record.sortOrder, record.enabled, record.extraJson)
        }
        ...
        return CmsListItem(requireNotNull(record.id), record.listId, sourceType, article.id, article.articleType, article.status, article.title, record.subtitle, if (article.articleType == ArticleType.EXTERNAL_LINK) article.externalUrl else null, null, record.imageResourceId, effectiveImage, record.openMode, record.sortOrder, record.enabled, record.extraJson)
    }
    ...
    private fun CmsListItemDraft.record(listId: Long, id: Long? = null) = CmsListItemRecord(id = id, listId = listId, sourceType = sourceType.name, articleId = articleId, title = title, subtitle = subtitle, url = url, imagePath = imagePath, imageResourceId = imageResourceId, openMode = openMode, sortOrder = sortOrder, enabled = enabled, extraJson = extraJson)
