...
class StaticResourceService(@Value("\${cms.static.root:./data/static}") rootText:String){
 ...
 fun list(path:String=""):List<StaticEntry>{val dir=safeDirectory(path);if(!Files.exists(dir))throw StaticResourceNotFoundException(path);if(!Files.isDirectory(dir))throw StaticResourceValidationException("指定路径不是目录");return Files.list(dir).use{stream->stream.filter{!it.startsWith(trashRoot)}.map{entry(it)}.sorted(compareBy<StaticEntry>{!it.directory}.thenBy{it.name}).toList()}}
 fun upload(path:String,file:MultipartFile,replace:Boolean):StaticEntry{if(file.isEmpty)throw StaticResourceValidationException("上传文件不能为空");val relative=normalizeFile(path);checkExtension(relative);val target=safeFile(relative);Files.createDirectories(target.parent);if(Files.exists(target)&&!replace)throw StaticResourceValidationException("目标文件已存在，请明确选择替换");file.inputStream.use{Files.copy(it,target,StandardCopyOption.REPLACE_EXISTING)};return entry(target)}
 fun delete(path:String):TrashEntry{val relative=normalizeFile(path);val source=safeFile(relative);if(!Files.isRegularFile(source))throw StaticResourceNotFoundException(path);val id=UUID.randomUUID().toString();Files.move(source,trashRoot.resolve("$id.data"),StandardCopyOption.REPLACE_EXISTING);Files.writeString(trashRoot.resolve("$id.meta"),relative);return TrashEntry(id,relative)}
 ...
 fun restore(id:String):StaticEntry{if(!id.matches(Regex("[0-9a-f-]{36}")))throw StaticResourceValidationException("回收记录无效");val data=trashRoot.resolve("$id.data");val meta=trashRoot.resolve("$id.meta");if(!Files.isRegularFile(data)||!Files.isRegularFile(meta))throw StaticResourceNotFoundException(id);val target=safeFile(Files.readString(meta));if(Files.exists(target))throw StaticResourceValidationException("原路径已有文件，不能直接恢复");Files.createDirectories(target.parent);Files.move(data,target);Files.deleteIfExists(meta);return entry(target)}
 fun resolvePublic(path:String):Path{val relative=normalizeFile(path);val p=safeFile(relative);if(!Files.isRegularFile(p))throw StaticResourceNotFoundException(path);return p}
 ...
 private fun normalizeFile(raw:String):String{val v=raw.trim().replace('\\','/').trimStart('/');if(v.isBlank()||v.split('/').any{it==".."||it=="."||it.isBlank()})throw StaticResourceValidationException("静态资源路径不合法");if(v.startsWith(".trash"))throw StaticResourceValidationException("不能操作回收区内部路径");return v}
 private fun safeFile(relative:String):Path{val p=root.resolve(relative).normalize();if(!p.startsWith(root)||p.startsWith(trashRoot))throw StaticResourceValidationException("静态资源路径越界");return p}
 private fun safeDirectory(raw:String):Path{if(raw.isBlank())return root;val v=raw.trim().replace('\\','/').trim('/');if(v.split('/').any{it==".."||it=="."||it.isBlank()})throw StaticResourceValidationException("静态资源路径不合法");val p=root.resolve(v).normalize();if(!p.startsWith(root)||p.startsWith(trashRoot))throw StaticResourceValidationException("静态资源路径越界");return p}
