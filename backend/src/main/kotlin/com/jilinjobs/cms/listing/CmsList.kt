...
data class CmsListDefinition(val id:Long,val code:String,val name:String,val groupCode:String,val description:String,val sortOrder:Int,val enabled:Boolean,val system:Boolean)
...
data class PublicCmsList(val id:Long,val code:String,val name:String,val groupCode:String,val items:List<CmsListItem>)
data class CmsListDraft(val code:String,val name:String,val groupCode:String="GENERAL",val description:String="",val sortOrder:Int=0,val enabled:Boolean=true,val system:Boolean=false)
...
data class CmsListRecord(var id:Long?=null,var code:String="",var name:String="",var groupCode:String="GENERAL",var description:String="",var sortOrder:Int=0,var enabled:Boolean=true,var systemFlag:Boolean=false)
...
interface CmsListMapper{
 @Select("SELECT id,code,name,group_code,description,sort_order,enabled,system_flag FROM cms_list ORDER BY group_code,sort_order,id") fun findAll():List<CmsListRecord>
 @Select("SELECT id,code,name,group_code,description,sort_order,enabled,system_flag FROM cms_list WHERE enabled=1 ORDER BY group_code,sort_order,id") fun findEnabled():List<CmsListRecord>
 @Select("SELECT id,code,name,group_code,description,sort_order,enabled,system_flag FROM cms_list WHERE id=#{id}") fun findById(@Param("id") id:Long):CmsListRecord?
 @Select("SELECT id,code,name,group_code,description,sort_order,enabled,system_flag FROM cms_list WHERE code=#{code}") fun findByCode(@Param("code") code:String):CmsListRecord?
 @Insert("INSERT INTO cms_list(code,name,group_code,description,sort_order,enabled,system_flag) VALUES(#{code},#{name},#{groupCode},#{description},#{sortOrder},#{enabled},#{systemFlag})") @Options(useGeneratedKeys=true,keyProperty="id") fun insertList(record:CmsListRecord):Int
 @Update("UPDATE cms_list SET name=#{name},group_code=#{groupCode},description=#{description},sort_order=#{sortOrder},enabled=#{enabled},system_flag=#{systemFlag} WHERE id=#{id}") fun updateList(record:CmsListRecord):Int
...
class CmsListService(private val mapper:CmsListMapper,private val objectMapper:ObjectMapper){
 ...
 @Transactional(readOnly=true) fun publicLists()=mapper.findEnabled().map{r->PublicCmsList(requireNotNull(r.id),r.code,r.name,r.groupCode,mapper.findEnabledItems(requireNotNull(r.id)).map{it.model()})}
 ...
 @Transactional fun createItem(listId:Long,d:CmsListItemDraft):CmsListItem{requireList(listId);val r=normalizeItem(d).record(listId);mapper.insertItem(r);return r.model()}
 @Transactional fun updateItem(listId:Long,id:Long,d:CmsListItemDraft):CmsListItem{requireList(listId);val c=mapper.findItem(id)?:throw CmsListItemNotFoundException(id);if(c.listId!=listId)throw CmsListValidationException("列表项不属于当前列表");mapper.updateItem(normalizeItem(d).record(listId,id));return mapper.findItem(id)!!.model()}
 ...
 private fun normalizeList(d:CmsListDraft):CmsListDraft{val code=d.code.trim().uppercase();if(!code.matches(Regex("[A-Z][A-Z0-9_]{1,99}")))throw CmsListValidationException("列表 Code 格式不正确");val name=d.name.trim();if(name.isBlank())throw CmsListValidationException("列表名称不能为空");val group=d.groupCode.trim().uppercase().ifBlank{"GENERAL"};if(!group.matches(Regex("[A-Z][A-Z0-9_]{1,49}")))throw CmsListValidationException("列表分组 Code 格式不正确");return d.copy(code=code,name=name,groupCode=group,description=d.description.trim())}
 private fun normalizeItem(d:CmsListItemDraft):CmsListItemDraft{val title=d.title.trim();if(title.isBlank())throw CmsListValidationException("列表项标题不能为空");val mode=d.openMode.trim().uppercase();if(mode !in openModes)throw CmsListValidationException("列表项打开方式不正确");val url=d.url?.trim()?.takeIf{it.isNotBlank()};val image=d.imagePath?.trim()?.takeIf{it.isNotBlank()};if(url!=null)validateUrl(url);if(image!=null&&!image.startsWith("/static/"))throw CmsListValidationException("列表项图片必须使用 /static/ 资源路径");val extra=d.extraJson?.trim()?.takeIf{it.isNotBlank()};if(extra!=null)runCatching{objectMapper.readTree(extra)}.getOrElse{throw CmsListValidationException("列表项扩展数据必须是合法 JSON")};return d.copy(title=title,subtitle=d.subtitle?.trim()?.takeIf{it.isNotBlank()},url=url,imagePath=image,openMode=mode,extraJson=extra)}
 ...
 private fun CmsListDraft.record(id:Long?=null)=CmsListRecord(id,code,name,groupCode,description,sortOrder,enabled,system)
 ...
 private fun CmsListRecord.model()=CmsListDefinition(requireNotNull(id),code,name,groupCode,description,sortOrder,enabled,systemFlag)
...
data class SaveCmsListRequest(val code:String,val name:String,val groupCode:String="GENERAL",val description:String="",val sortOrder:Int=0,val enabled:Boolean=true,val system:Boolean=false){fun draft()=CmsListDraft(code,name,groupCode,description,sortOrder,enabled,system)}
