...
data class NavigationLocation(val id:Long,val code:String,val name:String,val description:String,val sortOrder:Int,val enabled:Boolean,val system:Boolean)
...
data class NavigationLocationRecord(var id:Long?=null,var code:String="",var name:String="",var description:String="",var sortOrder:Int=0,var enabled:Boolean=true,var systemFlag:Boolean=false)
class NavigationLocationNotFoundException(code:String):RuntimeException("导航位置不存在：$code")

@Mapper
interface NavigationLocationMapper{
 @Select("SELECT id,code,name,description,sort_order,enabled,system_flag FROM cms_navigation_location ORDER BY sort_order,id") fun findAll():List<NavigationLocationRecord>
 @Select("SELECT id,code,name,description,sort_order,enabled,system_flag FROM cms_navigation_location WHERE code=#{code}") fun findByCode(@Param("code") code:String):NavigationLocationRecord?
...
class NavigationLocationService(private val mapper:NavigationLocationMapper,private val navigationMapper:NavigationMapper){
 ...
 @Transactional fun create(d:NavigationLocationDraft):NavigationLocation{val n=normalize(d);if(mapper.findByCode(n.code)!=null)throw NavigationValidationException("导航位置 Code 已存在：${n.code}");val r=n.record();mapper.insert(r);return r.model()}
 ...
 @Transactional fun delete(code:String){val c=normalizeCode(code);mapper.findByCode(c)?:throw NavigationLocationNotFoundException(c);if(navigationMapper.countByPosition(c)>0)throw NavigationValidationException("该导航位置仍包含导航条目，不能删除");mapper.delete(c)}
 ...
 private fun NavigationLocationRecord.model()=NavigationLocation(requireNotNull(id),code,name,description,sortOrder,enabled,systemFlag)
}
