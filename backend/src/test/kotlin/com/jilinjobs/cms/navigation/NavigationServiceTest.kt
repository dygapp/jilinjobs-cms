...
class NavigationServiceTest {
    ...
    fun `栏目目标必须引用已有栏目`() {
        ...
        val error = assertThrows(NavigationValidationException::class.java) {
            service.create(NavigationDraft("不存在栏目", "MAIN", null, NavigationTargetType.COLUMN, 999, null, 0, true))
        }
    ...
    fun `站内链接目标保留为站内路径`() {
        ...
        service.create(NavigationDraft("未来文章", "HOME_SHORTCUT", null, NavigationTargetType.LINK, null, "/articles/12", 0, true))
    ...
    fun `父子导航必须属于同一位置`() {
        ...
        assertThrows(NavigationValidationException::class.java) {
            service.create(NavigationDraft("跨位置子项", "HOME_SHORTCUT", null, NavigationTargetType.LINK, null, "/x", 0, true, parentId = parent.id))
        }
    }
}
...
private class InMemoryNavigationRepository : NavigationRepository {
    ...
    override fun insert(draft: NavigationDraft): CmsNavigation { val id = ++sequence; return draft.toModel(id).also { data[id] = it } }
...
private class FakeNavigationLocationMapper(vararg codes: String) : NavigationLocationMapper {
    ...
    override fun update(record: NavigationLocationRecord): Int { val current = rows[record.code] ?: return 0; rows[record.code] = record.copy(id = current.id); return 1 }
