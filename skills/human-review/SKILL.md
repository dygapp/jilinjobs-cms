...
2. 只读取当前评审直接需要的真实 semantic owner，不为了“完整”默认扫描全库、archive 或无关实现。
...
7. 基于重新读取后的 Current Authority 重新校准评审草稿，确认没有长期事实只存在于草稿、图形或会话中。
