...
path = Path("docs/technical/cms-site-package-boundary.md")
text = path.read_text(encoding="utf-8")
anchor = """EU-40 不包含视觉 / Product Intent 变化，上述 Current Evidence 与 final diff/Authority review 足以关闭本 Unit。

## 9. Completed slices"""
assert text.count(anchor) == 1, text.count(anchor)
restored = """EU-40 不包含视觉 / Product Intent 变化，上述 Current Evidence 与 final diff/Authority review 足以关闭本 Unit。
...
## 9. Completed slices"""
text = text.replace(anchor, restored)
...
print("EU40_TECHNICAL_AUTHORITY_REPAIR PASS")
